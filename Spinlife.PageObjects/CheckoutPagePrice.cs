...
namespace Spinlife.PageObjects
{
    public class CheckoutPagePrice : PageObject
    {
            ...
            string[] validUSZipCodes =
        {
            ...
            int firstDigit = random.Next(3, 10); // 3 to 9
            string phoneNumber = firstDigit.ToString() + random.Next(100000000, 1000000000).ToString();
            textBoxPhone.SendKeys(phoneNumber);
        ...
        public void FillPaypalDetails()
        {
            ...
            wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(btnNext));
            btnNext.Click();
        ...
        public void SelectPayTomorrow()
        {
            ...
            Thread.Sleep(2000);
             Console.WriteLine("Switching to Pay Tomorrow window");
            // wait.Until(d => _driver.WindowHandles.Count > 1);
            // _driver.SwitchTo().Window(_driver.WindowHandles[^1]);
            // Console.WriteLine(_driver.WindowHandles.Count);
            wait.Until(d => _driver.WindowHandles.Count > 1);
            var windowHandles = _driver.WindowHandles;
            _driver.SwitchTo().Window(windowHandles[windowHandles.Count - 1]);
            Thread.Sleep(3000);
            Console.WriteLine(_driver.WindowHandles.Count);
